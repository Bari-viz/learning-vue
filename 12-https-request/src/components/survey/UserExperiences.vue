...
export default {
	...
	data() {
		return {
			...
			error: null,
			databaseUrl:
				'https://vue-http-request-demo-de09a-default-rtdb.firebaseio.com/surveys.json',
		};
	},
	methods: {
		async loadExperiences() {
            
			fetch ('https://vue-http-request-demo-de09a-default-rtdb.firebaseio.com/surveys.json').then(function(response) {
                if (response.ok) {
            ...
            for (const id in data) {
                results.push({ id: id, name: data[id] });
            }
        });
